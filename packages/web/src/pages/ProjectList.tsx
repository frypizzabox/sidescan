...
export function ProjectList() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-10">
      ...
      <p className="text-[13px] text-ink-3 mt-0.5 mb-8">
        Configure projects in{" "}
        <code className="px-1 py-0.5 rounded bg-surface-sunk text-ink-2 font-mono text-[12px]">
          config.yaml
        </code>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {data?.projects.map((p) => (
          <Link
            ...
            className="group text-left rounded-md bg-surface-raised ring-1 ring-inset ring-hairline hover:ring-hairline-strong hover:shadow-sm transition-all p-4"
          >
            <div className="flex items-start gap-2">
              <div className="min-w-0 flex-1">
                ...
                {(p.description || p.aiInferredSummary) && (
                  <p className="text-[12px] text-ink-3 line-clamp-2 mt-0.5">
                    {p.description || p.aiInferredSummary}
                  </p>
                )}
              </div>
            ...
            <div className="mt-3 flex items-center gap-3 text-[11px] text-ink-4 font-mono">
              <span>
                {p.scan.frequency}
